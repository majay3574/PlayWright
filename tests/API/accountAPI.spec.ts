...
test(`getting account VAlue `, async ({ request }) => {
    const opppurl = `https://qeagle8-dev-ed.develop.my.salesforce.com/services/data/v36.0/sobjects/${product}`
...
data.forEach(accounts => {
    test(`deleting opportunity with id ${accounts.Id}`, async ({ request }) => {
        const oppurl = `https://qeagle8-dev-ed.develop.my.salesforce.com/services/data/v36.0/sobjects/${product}/` + accounts.Id;
        console.log(oppurl)
